...
type config struct {
	...
	// Add a new limiter struct containing fields for the requests-per-second and brust
	// values, and a boolean field which we can use to enable/disable rate limiting altogather.
	limiter struct {
		rps     float64
		burst   int
		enabled bool
...
func main() {
	...
	// Call app.serve() to start tthe server
	err = app.serve()
	if err != nil {
		logger.Error(err.Error())
		os.Exit(1)
